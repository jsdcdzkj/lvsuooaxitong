<template>
	<view class="list-wrap">
		<hh-nav-bar image="../../../../static/icon-nav-bg.png" title="待办事项" :show-back="true" status-theme="black"></hh-nav-bar>
...
let { list, pagnation, reset, extraParams, scrollLoad } = usePagnation(api.caseManage.toDo, {approveType: '', order: 2});
...
const isShow = (detailInfo) => {
	...
	return (detailInfo.letter == 2 || detailInfo.invoice == 2) && (detailInfo.approveIds?.approverType == 1 && detailInfo.approveIds?.approverValue.split(',').includes(`${store.userInfo.id}`)
	 || (detailInfo.approveIds?.approverType == 2 && hasPermissions))
}
const toPage = (item) => {
	let url = ''
	if(item.processType ==1 && item.letter == 2) {
		url = '/pages/letter/letterCheck?id=' + item.summonsId + '&caseId=' + item.id  + '&isOwner=' + `${item.createUser == store.userInfo.id}`
	}else if(item.invoice == 2) {
		url = '/pages/invoice/invoiceCheck?id=' + item.invoiceId + '&caseId=' + item.id  + '&isOwner=' + `${item.createUser == store.userInfo.id}` + '&isCust=' + !!item.cust
	}else if(item.cust) {
		url = '/pages/invoice/invoiceCheck?id=' + item.cust.id + '&caseId=' + item.cust.id  + '&isOwner=' + `${item.cust.createUser == store.userInfo.id}` + '&isCust=' + !!item.cust + '&invoice=' + item.cust.invoice
	}
	uni.navigateTo({
		url
	})
