<template>
	<view class="wrap">
		<hh-nav-bar image="../../../../static/icon-nav-bg.png" title="忘记密码" :show-back="true" status-theme="black"></hh-nav-bar>
	...
	const SubmitEvent = () => {
		baseForm.value
			...
			.then((res) => {
				...
				const { loginName, phone, confirmpass } = formData;
				api.user.updUser
					.resetPassword({
						loginName,
						phone,
...
// 						icon: 'none'
// 					});
// 					const { loginName, phone, confirmpass } = this.formData;
// 					api.user.updUser
// 						.resetPassword({
// 							loginName,
// 							phone,
