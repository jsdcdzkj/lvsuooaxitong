    ...
    <div class="app-container">
        ...
        <div class="table-wrap">
            ...
            >
                ...
                <!-- <el-table-column prop="caseTypeStr" width="130" label="案件类型" align="center"> </el-table-column> -->
                <!-- <el-table-column prop="content" label="发票内容" align="center"> </el-table-column> -->
                <el-table-column prop="money" show-overflow-tooltip width="160px" label="合同金额" align="center">
                    <template slot-scope="scope">
                        {{scope.row.money && Number(scope.row.money).toFixed(2)}}
                    </template>
                </el-table-column>
                <el-table-column prop="invoiceMoney" show-overflow-tooltip width="160px" label="开票金额" align="center">
                    <template slot-scope="scope">
                ...
                <!-- <el-table-column show-overflow-tooltip prop="mail" width="160px" label="电子邮箱" align="center"> </el-table-column> -->
                <el-table-column show-overflow-tooltip width="160px" prop="addressI" label="收件地址" align="center">
                    <template  slot-scope="scope">{{scope.row.addressI || '--'}}</template>
                </el-table-column>
                <el-table-column show-overflow-tooltip width="160px" prop="mail" label="申请开票日期" align="center">
                    <template  slot-scope="scope">{{scope.row.createTime || '--'}}</template>
                </el-table-column>
                <el-table-column width="160px" show-overflow-tooltip prop="invoiceTime" label="开票日期" align="center">
                    <template  slot-scope="scope">{{(scope.row.invoiceTime && scope.row.invoiceTime.substring(0,10)) || '--'}}</template>
                </el-table-column>
                <el-table-column v-if="isShowUpload()" fixed="right" prop="invoice" label="付款状态" align="center">
                    <template slot-scope="scope">
                        <el-switch v-model="scope.row.payType" active-value="1" inactive-value="2" @change="handleOperate(scope.row)"></el-switch>
                    </template>
                </el-table-column>
                <el-table-column fixed="right" prop="invoice" label="开票状态" align="center">
                    <template slot-scope="scope">
                        {{invoiceTypeList[scope.row.invoice - 1]}}
                    </template>
                </el-table-column>
                <el-table-column prop="invoiceNum" label="开票次数" align="center" fixed="right">
                        <template slot-scope="scope">
                ...
                <el-table-column fixed="right" label="操作" align="center" width="120">
                    <template slot-scope="scope">
                        ...
                        <template v-if="scope.row.createUser == userInfo.id">
                            <el-button
                                size="mini"
                                v-if="(scope.row.invoice == 1 || scope.row.invoice == 3)"
...
import { payType, getEntity, getPageList, getInvoiceList, toExportTemplate } from '@/api/invoice';
...
import { createViewInvoiceNumDialog } from '@/views/common/dialog';
...
export default {
    ...
    methods: {
        isShowUpload() {
            console.log('2222222222222222222roles', this.roles, this.roles.some(res => res.roleCode == '011' || res.roleCode == '010'))
        ...
        isShow(row) {
            ...
            return row.invoice == 2 && ((row.approveIds?.approverType == 1 && row.approveIds?.approverValue.split(',').includes(`${this.userInfo.id}`))
        ...
        search() {
            this.listQuery.invoice = this.invoice?.join(',')
            this.handlerSearch()
        },
        refresh() {
            this.invoice = []
        ...
        handleOperate(row) {
            const info = row.payType == 1 ? '已付款' : '未付款'
            this.$confirm(`您确认调整付款状态为${info}吗？`, "提示", {
                confirmButtonText: "确认",
                cancelButtonText: "取消",
                type: "warning"
            }).then(() => {
                payType({id: row.invoiceId, payType: row.payType}).then(res => {
                    this.$message({
                        type: "success",
                        message: `${info}成功!`
                    });
                    ...
                    row.payType == 1 ? row.payType = '2' : row.payType = '1'
                })
            }).catch(() => {
                row.payType == 1 ? row.payType = '2' : row.payType = '1'
            })
