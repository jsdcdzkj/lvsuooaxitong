<template>
    <el-drawer :visible.sync="dialogFormVisible" :size="variables.drawWidth" @close="close">
        ...
        <div class="drawer-content">
            ...
            <div class="mt15">
                <el-descriptions :column="2" border>
                    <el-descriptions-item
                        ...
                        >{{ formInline.name }}</el-descriptions-item
                    >
                    <!--        <el-descriptions-item label="任务权限" content-class-name="my-content">正常</el-descriptions-item>-->
                    <el-descriptions-item
                        label="争议类别"
                        label-class-name="my-label"
                        content-class-name="my-content"
                        >{{ formInline.groupDesc }}</el-descriptions-item
                    >
                </el-descriptions>
            </div>

            <div class="view-title mt15">
                <h4>申请人信息</h4>
            </div>
            <el-table
            ...
            >
                ...
                <el-table-column show-overflow-tooltip label="组角色" align="center" width="160">
                    <template slot-scope="scope">
                        <el-select
                            v-show="scope.row.isOK"
                            v-model="scope.row.roleId"
                            placeholder="请选择组角色"
                            style="width: 100%"
                            @change="changeRole(scope.row, scope.$index)"
                        >
                            <el-option
                        ...
                        <el-tag v-show="!scope.row.isOK" v-else type="success">{{
                            scope.row.infoD
                        }}</el-tag>
                    </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip label="操作" width="140" align="center">
                    <template slot-scope="scope">
                        <el-button
                            type="primary"
                            v-if="!scope.row.isOK"
                            @click="editVersion(scope.row, scope.$index)"
                            plain
                            size="mini"
                            >编辑
                        </el-button>
                        <el-button
                            type="success"
                            v-else
                            @click="saveVersion(scope.row, scope.$index)"
                            plain
                            size="mini"
                            >保存
                        </el-button>
        ...
        <member
            ref="member"
            @fetch-data=""
            @choosePeople="handleCustomEvent"
            :skill-group-list="this.list"
        ></member>
    </el-drawer>
</template>
    ...
    data() {
        return {
            ...
            rules: {
                ...
                groupDesc: [{ required: true, message: '请输入班组说明', trigger: 'blur' }],
            },
            list: [
                {
                    id: 1,
    ...
    methods: {
        ...
        save(formName) {
            ...
            for (var i = 0; i < this.list.length; i++) {
                if (
                    this.list[i].roleId === null ||
                    this.list[i].roleId === '' ||
                    this.list[i].roleId === undefined
                ) {
                    count++;
                }
